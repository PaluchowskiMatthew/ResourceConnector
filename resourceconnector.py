...
class FlaskAPIThread(threading.Thread):
    ...
    def run(self):
        ...
        # global status_message     #made global here
        # while True:
        #     c.acquire()
        #     if status_progress == 0:
        #         print "A: val=" + str(status_message)
        #         time.sleep(0.1)
        #         status_progress = 1
        #         status_message = 30
        #         c.notify_all()
...
class ScriptWrapperThread(threading.Thread):
    def __init__(self, name):
        threading.Thread.__init__(self)
        self.name = name
    ...
    def run(self):
        ...
        while True:
            ...
            if status_progress == 1:
                print "B: val=" + str(status_message)
                time.sleep(0.5)
                status_progress = 0
                status_message = 20
                c.notify_all()
    ...
    def launch_script(command, resource_stdout):
        # global resource_stdout
        # punchball = 'woop!'
...
if __name__ == "__main__":
    ...
    script_thread = threading.Thread(name= 'Resource-Script-Thread',target=launch_script, args=(script_command, resource_stdout))
    script_thread.start()

    a = FlaskAPIThread("FlaskAPIThread")
    b = ScriptWrapperThread("ScriptWrapperThread")
